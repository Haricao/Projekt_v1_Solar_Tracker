...
uint8_t ADC_Init(void)
{
	...
	PORTA->PCR[12] &= ~(PORT_PCR_MUX(0));		//PTA12 - wejscie analogowe, Multiplekser na 0
	PORTA->PCR[9] &= ~(PORT_PCR_MUX(0));		//PTA9 - wejscie analogowe, Multiplekser na 0
	PORTA->PCR[8] |= PORT_PCR_MUX(0);				//PTA8 - wejscie analogowe, Multiplekser na 0
	PORTA->PCR[7] |= PORT_PCR_MUX(0);				//PTA7 - wejscie analogowe, Multiplekser na 0
	...
	ADC0->SC3  = ADC_SC3_AVGE_MASK | ADC_SC3_AVGS(3);																						// Wlacz usrednianie na 32 (3)
